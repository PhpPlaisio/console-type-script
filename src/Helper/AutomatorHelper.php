...
namespace Plaisio\Console\TypeScript\Helper;
...
class AutomatorHelper
{
  ...
  private static array $masks = [1          => ['IN_ACCESS', 'File was accessed (read)'],
                                 ...
                                 1073742080 => ['IN_CREATE',
                                                'High-bit: File or directory created in watched directory'],
                                 1073742336 => ['IN_DELETE',
                                                'High-bit: File or directory deleted in watched directory'],
                                 16777216   => ['IN_ONLYDIR',
  ...
  public function once(): void
  {
    ...
    $helper = new FixHelper($this->io, $this->jsAssetPath);
  ...
  {
    $oldLines = MarkHelper::sourceAsLines($path);
    $tmp      = $this->rewriteImportsAndExports(Path::getDirectory($path), $oldLines);
    $newLines = MarkHelper::appendHashToLines($tmp);
  ...
  private function mustTranspile(string $tsPath): bool
  {
    ...
    $tsHash = MarkHelper::computeHashFromSource($tsPath);
    $jsHash = MarkHelper::extractHashFromSource($jsPath);
